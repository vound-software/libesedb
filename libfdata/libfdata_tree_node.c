     ...
     liberror_error_t **error )
{
	...
	/*{
		static int append_node_counter = 0;
		append_node_counter++;
		printf ("Appended %d tree nodes.\n", append_node_counter);
	}*/
...
__declspec(dllexport) FILE *debug_msg_file = NULL;

int num_of_invocations = 0;
     ...
     liberror_error_t **error )
{
	...
	// static int 
	/*
	if (++num_of_invocations % 10000 == 0)
	{
		printf ("libfdata_tree_node_read_leaf_node_values invoked %d times.\n", num_of_invocations);
	}
	*/
	if( node == NULL )
	{
	 ...
	 && ( ( internal_tree_node->flags & LIBFDATA_TREE_NODE_FLAG_SUB_NODES_READ ) == 0 ) )
	{
		...
		if( ( internal_tree_node->flags & LIBFDATA_TREE_NODE_FLAG_SUB_NODES_RANGE_SET ) != 0 )
		{			
			// NK: THis method reads subnodes (fast)
			if( libfdata_tree_read_sub_nodes(
			     internal_tree_node->tree,
			     file_io_handle,
			     cache,
	...
	// printf ("Reading %d subnodes (%p).\n", number_of_sub_nodes, node);
	// Iterates over the set of subnodes...
	for( sub_node_index = 0;
	     sub_node_index < number_of_sub_nodes;
	     sub_node_index++ )
	{
		static unsigned long int reading_subnodes = 0;
		// printf ("Reading subnodes %d times", ++reading_subnodes);
		if( libfdata_array_get_entry_by_index(
		     internal_tree_node->sub_nodes,
		     sub_node_index,
		     (intptr_t **) &sub_node,
		...
		if (sub_node != NULL && debug_msg_file != NULL)
		{
			fprintf (debug_msg_file, "\"n%p\" ->  \"%p\"\n", node, number_of_sub_nodes, sub_node);
		}
 ...
 * Returns 1 if successful or -1 on error
 */
int libfdata_tree_node_read_leaf_node_values_cnt = 0;
...
//	invocation_counter++;
//	if (invocation_counter > max_recursion_depth) {
//		max_recursion_depth = invocation_counter;
//		printf ("Achieved max recursion depth: %d\n", max_recursion_depth);
//	}
