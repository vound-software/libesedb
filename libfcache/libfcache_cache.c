     ...
     liberror_error_t **error )
{
	...
	//printf ("Initializing cache: num_entries = %d\n", maximum_cache_entries);
	...
	*cache = (libfcache_cache_t *) internal_cache;
	/*
	printf ("Initialized cache: = %d / %d / %d\n",
		internal_cache->number_of_cache_values, 
		internal_cache->entries->number_of_allocated_entries,
		internal_cache->entries->number_of_entries);
		*/
	return( 1 );
     ...
     liberror_error_t **error )
{
	...
	if( *cache != NULL )
	{
		...
		*cache         = NULL;
		// printf ("Deallocated cache: num_entries = %d / %d \n", internal_cache->number_of_cache_values, internal_cache->entries->number_of_allocated_entries);
     ...
     liberror_error_t **error )
{
	...
	internal_cache = (libfcache_internal_cache_t *) cache;
	// printf ("Deleted cache data: num_entries = %d\n", internal_cache->number_of_cache_values);
 ...
 * Returns 1 if successful or -1 on error
 */
int libfcache_cache_get_value_by_index_cnt = 0;
     ...
     liberror_error_t **error )
{
	...
	internal_cache = (libfcache_internal_cache_t *) cache;
	/*
	if (++libfcache_cache_get_value_by_index_cnt % 1000000)
	{
		if (internal_cache->entries->number_of_allocated_entries == 
			internal_cache->number_of_cache_values) {
				printf ("Cache full: %d / %d / %d\n",
					internal_cache->number_of_cache_values,
					internal_cache->entries->number_of_entries,
					internal_cache->entries->number_of_allocated_entries);
					
		}
		libfcache_cache_get_value_by_index_cnt = 0;
	}
	*/
